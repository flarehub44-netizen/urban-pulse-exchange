-- V07: KYC cumulative withdrawal gate
--
-- Attack: user without KYC calls request_withdrawal(99) many times.
-- Old guard only checked the single transaction amount (>100), so each
-- individual R$99 withdrawal passed. With N accounts this scales linearly.
--
-- Fix: sum all successful withdrawals in the current calendar month.
-- If (monthly_total + this_request) > 100 and KYC not approved → reject.
-- A single R$99 withdrawal still passes; a second one in the same month is blocked.
...
begin
  ...
  -- Sum all withdrawals already processed this calendar month (paid status only)
