-- Atomic bet placement: validates balance, updates pool, writes bet + transaction in one transaction.
-- Uses security definer so it can touch multiple tables bypassing RLS,
-- while still binding to auth.uid() for ownership.
