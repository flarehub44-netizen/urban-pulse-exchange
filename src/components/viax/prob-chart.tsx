...
export function ProbChart({ m, height = 280 }: { m: Market; height?: number }) {
  const data = m.history.map((h) => ({
    ...
    <div style={{ width: "100%", height }}>
      <ResponsiveContainer>
        <AreaChart data={data} margin={{ top: 10, right: 16, left: -16, bottom: 0 }}>
          ...
          <YAxis domain={[0, 100]} tick={{ fill: "var(--color-muted-foreground)", fontSize: 10 }} axisLine={false} tickLine={false} width={36} />
          <Tooltip
            cursor={{ stroke: "var(--color-border)", strokeWidth: 1 }}
            contentStyle={{ background: "var(--color-popover)", border: "1px solid var(--color-border)", borderRadius: 12, fontSize: 12 }}
            labelStyle={{ color: "var(--color-muted-foreground)" }}
          />
          <Area type="monotone" dataKey="SIM" stroke="var(--color-up)" strokeWidth={1.6} fill="url(#gSim)" />
          <Area type="monotone" dataKey="NAO" stroke="var(--color-down)" strokeWidth={1.4} fill="url(#gNao)" />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
