...
// Seed markets for dev placeholder (TanStack Query) — not global Zustand state
const NOW = Date.now();
const min = 60_000;
...
interface ViaXState {
  ...
  // Animation tick — updates regions for visual simulation (markets via Query + market-tick)
  tick: () => void;
}
