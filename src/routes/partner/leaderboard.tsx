  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {(rows ?? []).map((r, i) => (
          <div key={r.partner_id} className="flex items-center gap-3 rounded-xl border bg-card/60 px-4 py-3">
            <span className="mono text-sm text-muted-foreground w-6">#{i + 1}</span>
            <div className="min-w-0 flex-1">
              <div className="font-medium truncate">{r.name}</div>
              <div className="text-xs text-muted-foreground">@{r.handle} · @{r.slug}</div>
            </div>
