      ...
      <div className="grid gap-5 lg:grid-cols-[1.6fr_1fr]">
        <CityHeatmap
          ...
          onRegionClick={(r) => {
            ...
            if (top) {
              navigate({ to: "/markets/$marketId", params: { marketId: top.id } });
            } else {
              navigate({ to: "/markets", search: { region: r.name } });
              toast.message("Nenhum mercado direto — lista filtrada por região");
            }
          }}
        />
          ...
          <div className="rounded-2xl border bg-card/60 p-4 backdrop-blur">
            ...
            <ul className="space-y-2">
              {regions.map((r) => {
                ...
                const dotColor =
                  tone === "text-down" ? "bg-down" : tone === "text-warn" ? "bg-warn" : "bg-up";
                return (
                  <li key={r.id}>
                    <button
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {r.name}
                        <ExternalLink className="size-3 text-muted-foreground/0 group-hover:text-muted-foreground transition" />
                      </div>
                      <div className="flex items-center gap-3 text-xs">
                        <span className="mono text-muted-foreground">
                          <AnimatedNumber value={r.flow} /> /h
                        </span>
                        <span className={`mono ${tone}`}>
                          <AnimatedNumber value={r.avgSpeed} decimals={0} /> km/h
                        </span>
                      </div>
                    </button>
                  </li>
          ...
          <div className="rounded-2xl border bg-card/60 p-4 backdrop-blur">
            ...
            <ul className="space-y-2 text-sm">
              {events.map((e, i) => (
                <li
                  key={i}
                  className="flex items-start gap-3 rounded-lg border bg-surface/40 p-2.5"
                >
              ...
              {markets.slice(0, 5).map((m) => (
                <li key={m.id}>
                  <Link
                    ...
                    className="flex items-center justify-between rounded-lg border bg-surface/40 p-2.5 text-sm hover:bg-surface"
                  >
